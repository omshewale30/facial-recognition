...
class CamApp(App):
    ...
    def update(self, *args):
        ...
        frame = frame[10:10 + 500, 900:900 + 500, :]

        buf1 = cv2.flip(frame, 0).tostring()
        img_texture = Texture.create(size=(frame.shape[1], frame.shape[0]), colorfmt='bgr')
        img_texture.blit_buffer(buf1, colorfmt='bgr', bufferfmt='ubyte')
    ...
    def verify(self):
        ...
        for img in os.listdir(os.path.join('application_data', 'verification_images')):
            input_img = self.preprocess(os.path.join('application_data', 'input_img', 'input_img.jpg'))
            validation_img = self.preprocess(os.path.join('application_data', 'verification_images', img))
        ...
        self.verification_label.text = 'Verification Successful' if verified else 'Verification Unsuccessful'
